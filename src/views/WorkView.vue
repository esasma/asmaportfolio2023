

<template>
  <div class="flex pt-16 items-center justify-center mb-36 ">
    <div class="grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-3">


      <router-link to="/ecarvtcwork">
        <div
          class="group relative cursor-pointer items-center justify-center overflow-hidden transition-shadow hover:shadow-xl hover:shadow-black/30">
          <div class="h-96 w-72">
            <img
              class="h-full w-full object-cover transition-transform duration-500 group-hover:rotate-3 group-hover:scale-125"
              src="../assets/ecarvtcgif.gif" alt="fond gif bleu" />
          </div>
          <div
            class="absolute inset-0 bg-gradient-to-b from-transparent via-transparent to-black group-hover:from-black/70 group-hover:via-black/60 group-hover:to-black/70">
          </div>
          <div
            class="absolute inset-0 flex translate-y-[60%] flex-col items-center justify-center px-9 text-center transition-all duration-500 group-hover:translate-y-0">
            <h1 class="font-dmserif text-3xl font-bold text-white mb-16 uppercase">Ecar Vtc</h1>
            <p class="mb-3 text-lg italic text-white opacity-0 transition-opacity duration-300 group-hover:opacity-100">
              Ecar-vtc est un projet créé en Wordpress.</p>
            <button
              class=" bg-neutral-900 py-2 px-3.5 font-com text-sm capitalize text-white shadow shadow-black/60 hover:bg-slate-800">En
              Savoir plus</button>
          </div>
        </div>
      </router-link>


      <router-link to="/Sigma">
        <div
          class="group relative cursor-pointer items-center justify-center overflow-hidden transition-shadow hover:shadow-xl hover:shadow-black/30">
          <div class="h-96 w-72">
            <img
              class="h-full w-full object-cover transition-transform duration-500 group-hover:rotate-3 group-hover:scale-125"
              src="../assets/gifbleu.gif" alt="gif" />
          </div>
          <div
            class="absolute inset-0 bg-gradient-to-b from-transparent via-transparent to-black group-hover:from-black/70 group-hover:via-black/60 group-hover:to-black/70">
          </div>
          <div
            class="absolute inset-0 flex translate-y-[60%] flex-col items-center justify-center px-9 text-center transition-all duration-500 group-hover:translate-y-0">
            <h1 class="font-dmserif text-3xl font-bold text-white mb-16 uppercase">Sigma </h1>
            <p class="mb-3 text-lg italic text-white opacity-0 transition-opacity duration-300 group-hover:opacity-100">
              Sigma est un projet créé en Wordpress. </p>
            <button
              class=" bg-neutral-900 py-2 px-3.5 font-com text-sm capitalize text-white shadow shadow-black/60 hover:bg-slate-800">En
              Savoir Plus</button>
          </div>
        </div>
      </router-link>


      <router-link to="/Wereplay">
        <div
          class="group relative cursor-pointer items-center justify-center overflow-hidden transition-shadow hover:shadow-xl hover:shadow-black/30">
          <div class="h-96 w-72">
            <img
              class="h-full w-full object-cover transition-transform duration-500 group-hover:rotate-3 group-hover:scale-125"
              src="../assets/Protobacillus-3.gif" alt="fond gif" />
          </div>
          <div
            class="absolute inset-0 bg-gradient-to-b from-transparent via-transparent to-black group-hover:from-black/70 group-hover:via-whitek/60 group-hover:to-black/70">
          </div>
          <div
            class="absolute inset-0 flex translate-y-[60%] flex-col items-center justify-center px-9 text-center transition-all duration-500 group-hover:translate-y-0">
            <h1 class="font-dmserif text-3xl font-bold text-white mb-16 uppercase">werplay</h1>
            <p class="mb-3 text-lg italic text-white opacity-0 transition-opacity duration-300 group-hover:opacity-100">
              Wereplay est un projet créé en Vuejs.</p>
            <button
              class=" bg-neutral-900 py-2 px-3.5 font-com text-sm capitalize text-white shadow shadow-black/60 hover:bg-slate-800">En
              Savoir plus</button>
          </div>
        </div>
      </router-link>

    </div>



  </div>
</template>


<script>

</script>



